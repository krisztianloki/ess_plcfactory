...
def WriteStandardPLCCode():
	...
	ExternalSourceFile.append("	                     BytesToSend := 134,")
...
def ProcessIFADevTypes(OutputDir, IfaName):
    
	IfaPath = os.path.join(OutputDir, IfaName)
	#Process IFA devices	
	...
	while pos < len(OrderedLines)-1:
		if OrderedLines[pos].rstrip() == "DEVICE":
			...
			if ActualDeviceType not in DeviceTypeList:
				...
				DevTypeBODY_HEADER.append("    //Input File Name: " + IfaName)
	...
	if IfaPath <> "":
		...
		if SclPath <> "":		
			...
			if HASH <> "" and DeviceNum <> 0:
				...
				WriteStandardPLCCode()
				theexternalsourcefile = open(os.path.join(OutputDir, "InterfaceFactory_external_source_standard.scl"), 'wb')
				for line in ExternalSourceFile:
					theexternalsourcefile.write(line+'\r\n')
				theexternalsourcefile.close()	
				ExternalSourceFile = []
				
				#Process devices/device types
				ProcessIFADevTypes(OutputDir,IfaName)			

				#WriteDiagnostics	
				WriteDiagnostics()	
				
				#Write the output fo file				
				theexternalsourcefile = open(os.path.join(OutputDir, "InterfaceFactory_external_source.scl"), 'wb')
				for line in ExternalSourceFile:
					theexternalsourcefile.write(line+'\r\n')
					
				print SclPath
				#Copy the .SCL from the ZIP file to the end of the code	
				with open(os.path.join(SclPath), 'r') as thesclsourcefile:	
					for line in thesclsourcefile:
						theexternalsourcefile.write( line)
				theexternalsourcefile.close()	

...
class BEAST_GDCA(BEAST_BASE):
    ...
    @staticmethod
    def __alarmslist(ls, f, level):
        for guidance in ls:
            guidance.toalarms(f, level)


    def toalarms(self, f, level):
        self.__alarmslist(self._guidances, f, level)
        self.__alarmslist(self._commands, f, level)
        self.__alarmslist(self._displays, f, level)
        self.__alarmslist(self._automated_actions, f, level)
...
class BEAST_DEF(object):
  ...
  Alarm configuration generated by AlarmFactory using the following arguments (please remove the spaces between the hyphens):
  {args}

...
It is possible to add new variables to `env.sh`; PLCFactory tries its best to keep those changes intact. However, `st.cmd` is always overwritten (except if `--no-ioc-st-cmd` is specified) although a backup of the custom `st.cmd` is created as `st.cmd.orig`. So, if you need a custom `st.cmd` it is better to generate it once with PLCFactory, customize it and then use `--no-ioc-st-cmd` to protect your changes; since all the PLC specific settings are in the `.iocsh` file the generated `st.cmd` would be the same anyway each time you run PLCFactory.

If you would like to add extra database files, `.iocsh` files or whatnot you have to "protect" them because PLCFactory **actively** removes files in subdirectories of the created IOC that were not generated during the current run. Protecting custom files is done by listing them in the `.plcfactory_ignore` file. Another solution is to simply use the autogenerated `custom.iocsh`. It is created as an empty file and is not deleted or modified in any way.
